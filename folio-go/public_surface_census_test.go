...
var publicSurfacePins = []string{
	...
	"folio8 const DiagCodeTextFaceSubstituted",
	"folio8 const FaceFallbackStrict",
	"folio8 const FaceFallbackSubstitute",
...
func TestOnlyRootAndFontsAreImportableLibraryPackages(t *testing.T) {
	...
	err := filepath.WalkDir(root, func(path string, d os.DirEntry, err error) error {
		...
		seen[rel] = true
		if !allowed[rel] {
			t.Errorf("UNEXPECTED importable package folio-go/%s (package %s, in %s) — it is public API the v1 census does not scan. Move it under internal/, or add it to the census deliberately.", rel, file.Name.Name, name)
		}
		return nil
	})
	if err != nil {
	...
	for dir := range allowed {
		if !seen[dir] {
			t.Errorf("EXPECTED library package folio-go/%s is GONE — the walk found no non-main Go source there, so this check is scanning the wrong tree", dir)
		}
	}
}
...
func TestPublicSurfaceMatchesTheFrozenV1Census(t *testing.T) {
	...
	for _, g := range got {
		if !slices.Contains(want, g) {
			t.Errorf("UNEXPECTED public identifier %q — folio-go's v1 API is frozen (D-1.1.c, RELEASING.md). If the addition is deliberate and reviewed, add it to publicSurfacePins in the same commit; otherwise unexport it.", g)
		}
	}
	for _, w := range want {
