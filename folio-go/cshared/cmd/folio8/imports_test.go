...
// contract folio-go/wasm/cmd/render is held to: it calls the public folio8
// API only, so it imports no folio-go/internal package, and it compiles in no
// fonts, so it does not import folio-go/fonts. A caller supplies every face,
// on every call, exactly as in Go.
func TestCSharedEntryImportsOnlyThePublicAPI(t *testing.T) {
	const module = "github.com/panitw/folio8/folio-go"
