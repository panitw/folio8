...
folio8 for Node. Turn a `.folio` template plus JSON data into PDF bytes
in-process, with the folio8 engine itself — compiled to WebAssembly — doing the
rendering.
...
and newer with `require('folio-js')`, which Node resolves through its
require(esm) support.
...
| `shipped()` — from `folio-js/fonts` | The shipped font set, as a `Map<string, Uint8Array>`. |
...
across folio-js, folio-dotnet and Go, so a caller can port between them.
...
folio-js itself is MIT — see `LICENSE`. The shipped faces are not: they are
OFL-1.1, as above.
