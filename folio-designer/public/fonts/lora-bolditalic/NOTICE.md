# Lora-BoldItalic.ttf — spec-install-all-face-cuts story 3, the committed tier's cuts
...
It is the **Bold Italic** cut of the committed family **Lora**, added by
spec-install-all-face-cuts story 3 so that a family behaves the same whichever
tier it came from (CAP-3). Its Regular is `../lora/`, and **both come from
...
the family **`Lora Bold Italic`** into `src/generated/runtime-fonts.css`.
...
`post.italicAngle` -3, **no** `fvar`/`gvar` and **no** `CFF`/`CFF2` —
...
| Path inside the archive | `ttf/Lora-BoldItalic.ttf` |
| Fetched | 2026-09-20 |
| sha256 of the release archive | `19061972d1124d258dffd41f3ad12ce2db513f9b31fb98ece7afe7b538e8647f` (2,443,935 bytes) |
| **sha256 of the SOURCE (upstream) file** | `f448e41ab0bf529ec7bd4f8aef843eb4d33b5608c7ed0a53128ead588e6ea9e5` |
...
| item | value |
|---|---|
| **Shipped file** | `Lora-BoldItalic.ttf` |
| **sha256 of the SHIPPED file** | `f448e41ab0bf529ec7bd4f8aef843eb4d33b5608c7ed0a53128ead588e6ea9e5` |
| Size | 213,560 bytes |
| Instance | Bold Italic — `OS/2.usWeightClass` 700, upstream static build |
| Declared family (`name` table) | `Lora` / `Bold Italic` |
...
family's Regular, copied from `../lora/LICENSE-OFL.txt`.
