# Montserrat-BoldItalic.ttf — spec-install-all-face-cuts story 3, the committed tier's cuts
...
It is the **Bold Italic** cut of the committed family **Montserrat**, added by
spec-install-all-face-cuts story 3 so that a family behaves the same whichever
tier it came from (CAP-3). Its Regular is `../montserrat/`, and **both come from
...
the family **`Montserrat Bold Italic`** into `src/generated/runtime-fonts.css`.
...
`post.italicAngle` -11.3, **no** `fvar`/`gvar` and **no** `CFF`/`CFF2` —
...
| Path inside the archive | `Montserrat-9.000/fonts/ttf/Montserrat-BoldItalic.ttf` |
| Fetched | 2026-09-20 |
| sha256 of the release archive | `991b3f971c65081c780fa15645a030bdf9e21ee8734b9f6010638b83bae1ff83` (30,979,460 bytes) |
| **sha256 of the SOURCE (upstream) file** | `c5c3196aace88eb63160ebe4bef7e45937d8bfc2c69d6d652ffdf082fe864ca0` |
...
| item | value |
|---|---|
| **Shipped file** | `Montserrat-BoldItalic.ttf` |
| **sha256 of the SHIPPED file** | `c5c3196aace88eb63160ebe4bef7e45937d8bfc2c69d6d652ffdf082fe864ca0` |
| Size | 460,160 bytes |
| Instance | Bold Italic — `OS/2.usWeightClass` 700, upstream static build |
| Declared family (`name` table) | `Montserrat` / `Bold Italic` |
...
family's Regular, copied from `../montserrat/LICENSE-OFL.txt`.
