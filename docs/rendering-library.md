...
bytes this library produces. Five companion references hold the rules this guide does not repeat:
...
- [Performance](performance.md) — measured throughput and memory, and how to size a publishing host.
...
Install the released module, `folio-go/v1.2.0`. From your application's directory:

```sh
go mod init example.com/folio8-demo
go get github.com/panitw/folio8/folio-go@v1.2.0
go mod tidy
```

`go get` records `github.com/panitw/folio8/folio-go v1.2.0` in your `go.mod`. The public API is
...
GitHub release notes for `folio-go/v1.2.0`, which list what changed.
...
Returns a new `folio8.FontSet` map holding the eleven faces embedded in the `fonts` package. Each face ships with its OFL-1.1 licence text and NOTICE under `folio-go/fonts/`. The keys are exactly:
...
`LocaleTableVersion` identifies the built-in locale formatting table; its current value is `1`. `Version` is the library version string, `"1.2.0"` for the `folio-go/v1.2.0` release.

## Command-line tool

The module also contains a `folio8` command with `validate` and `render` subcommands
(`go run github.com/panitw/folio8/folio-go/cmd/folio8@v1.2.0 render -data data.json -o out.pdf template.folio`).
