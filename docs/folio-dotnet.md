...
**What the engine does with signals on Linux, and what the binding does about it.** Loading the engine brings the Go runtime with it, and Go's start-up re-installs every signal handler it finds in the process with `SA_ONSTACK` added — including handlers the .NET runtime installed deliberately without it. One of those is the runtime's GC activation signal, whose handler is sized for a thread's ordinary stack; moved onto the runtime's fixed 16 KiB alternate stack, it overflows, and the process dies on a thread-pool worker during a garbage collection — a segfault, an abort, or the kernel's `overflowed sigaltstack`, depending on where the overflow lands. That is why 1.1.0 shipped no Linux native. From 1.2.0 the engine puts the runtime's own dispositions back itself, inside its own load: a constructor linked to run immediately after Go's start-up writes back the pre-load disposition of every handler the load re-flagged without replacing, and Go's own handlers are left alone. The binding records the same dispositions before the load and restores them again after its first call has returned, as a fallback, and a test reads both `sigaction` and the engine's own report back rather than trusting a run of clean renders. On Linux amd64 the binding's restore alone took a bare reproduction of the crash from 83 in 150 runs to 1; the remaining one to four in a hundred were the moments between Go's edit and that later restore, which the engine's constructor closes. The affected threads are the runtime's, which no amount of work on the binding's own threads could reach.

**The pool's threads additionally carry a large alternate stack of their own**, given once at creation, before their first crossing, so that a fault taken inside the engine on a thread the binding owns has room for both runtimes' handlers. Windows has no such signals and needs none of it, and takes the same call path anyway so that one path is exercised everywhere.
